...
export async function getServerSideProps({req, res}) {
  ...
  async function recentDatasets() {
    const response = await fetch('https://data.ca.gov/api/3/action/recently_changed_packages_activity_list',{headers: {'User-Agent': 'NextGenAPI/0.0.1',}}).then(response => response.json());
    return response.result
  }
  async function buildTopics() {
    const topicArray = []

    const response = await fetch('https://data.ca.gov/api/3/action/group_list',{headers: {'User-Agent': 'NextGenAPI/0.0.1',}}).then(response => response.json());
    ...
    for (let index = 0; index < group_array.length; index++) {
      ...
      const res =  await fetch('https://data.ca.gov/api/3/action/group_show?id='+groupName,{headers: {'User-Agent': 'NextGenAPI/0.0.1',}})
