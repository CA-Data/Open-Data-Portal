...
export async function getServerSideProps(context) {
  ...
  var topicIcn = (response.result.groups.length == 1 && response.result.groups[0].name && topicIconArray[response.result.groups[0].name]) ? topicIconArray[response.result.groups[0].name] : topicIconArray["general"];
                ...
                <p className="dataset-info-label">
                  ...
                  {data.group_object[0].title
                    ...
                    <span style={{display:"block"}}>
                      {data.group_object.map((topic, index) => (
                        <span key={topic.id}>
                          {topic.title}<br />
                        </span>
                      ))}
