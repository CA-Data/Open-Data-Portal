...
const getFormattedData =async(context)=>{
  ...
  const filters = await fetch(`https://data.ca.gov/api/action/package_search?${apirequest.split('?')[1]}&facet.field=["groups","tags","organization","res_format"]&rows=0`,{headers: {'User-Agent': 'NextGenAPI/0.0.1',}}).then(response=>response.json()).catch(error=>console.log(error))
