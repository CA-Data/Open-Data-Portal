...
const getFormattedData = async (context) => {
  ...
  if (!String.linkify) {
    String.prototype.linkify = function () {
      ...
      return this.replace(urlPattern, '<a href="$&">$&</a>').replace(
        pseudoUrlPattern,
        '$1<a href="http://$2">$2</a>'
      );
  ...
  if ("publisher" in context.query && context.query.publisher.length > 0) {
    ...
    const popular_datasets = await fetch(
      `https://data.ca.gov/api/3/action/package_search?q=${q}&sort=views_recent%20desc&fq=organization:${publisher}&rows=3`
    )
    ...
    if (popular_datasets.result.count > 0) {
      publisherDetails.title =
        popular_datasets.result.results[0].organization.title;
      publisherDetails.description =
        popular_datasets.result.results[0].organization.description
          .linkify()
          .replace(/(?:\r\n|\r|\n)/g, "<br>");
      ...
      <main id="body-content" className="cagov-main">
        ...
        >
          ...
          <div className="cagov-content content-cell">
                ...
                <div className="organization-info">
                  ...
                  {data.publisherDetails.website && (
                    <p>
                      ...
                      <a href={data.publisherDetails.website}>
                        {data.publisherDetails.title}
                      </a>
                    </p>
                  )}
                  <p
                    ...
                    dangerouslySetInnerHTML={{
                      __html: data.publisherDetails.description,
                    }}
                  ></p>
                </div>
                  ...
                  <div className="popular-datasets-cards-container">
                    {data.publisherDetails.popular &&
                      data.publisherDetails.popular.map((details, index) => (
                        ...
                        >
                          <p className="dataset-title">{details.title}</p>
                          <p>
                            <span className="large-text">{details.views}</span>{" "}
                            recent views
                          </p>
                        </a>
                      ))}
            ...
            <h3 style={{ marginTop: 0, color: "#034A6B" }}>
              {areObjectKeysEmpty(dataState.parameters)
                ? "All datasets"
                : `Search ${data.publisherDetails.title} datasets`}
            </h3>
