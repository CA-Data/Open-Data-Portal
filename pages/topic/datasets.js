...
const getFormattedData = async (context) => {
  ...
  if (context.query.topic && topicIconArray[context.query.topic]) {
    var apireqtopic =
      "https://data.ca.gov/api/3/action/package_search?rows=3&fq=groups:(" +
      context.query.topic +
    ...
    for (let index = 0; index < responsetopic.result.results.length; index++) {
      ...
      dataset.organization =
        responsetopic.result.results[index].organization.title;
      var apireqdataset =
        "https://data.ca.gov/api/3/action/package_show?name_or_id=" +
        dataset.name +
        "&include_tracking=true";
      const responseDataset = await fetch(apireqdataset).then(
        (responseDataset) => responseDataset.json()
      );
    ...
    for (
      let index = 0;
      index < responsetopic.result.results[0].groups.length;
      index++
    ) {
      if (
        context.query.topic ==
        responsetopic.result.results[0].groups[index].name
      ) {
        topicDisplayName =
          responsetopic.result.results[0].groups[index].display_name;
        topicDescription =
          responsetopic.result.results[0].groups[index].description;
      }
    }
...
const Results = (data) => {
  ...
  const [topicList, setTopicList] = useState(
    Object.entries(data.filters.result.facets.groups).sort((a, b) =>
      a[1] > b[1] ? -1 : 1
    )
  );
  const [publisherList, setPublisherList] = useState(
    Object.entries(data.filters.result.facets.organization).sort((a, b) =>
      a[1] > b[1] ? -1 : 1
    )
  );
  const [tagList, setTagList] = useState(
    Object.entries(data.filters.result.facets.tags).sort((a, b) =>
      a[1] > b[1] ? -1 : 1
    )
  );
  const [formatList, setFormatList] = useState(
    Object.entries(data.filters.result.facets.res_format).sort((a, b) =>
      a[1] > b[1] ? -1 : 1
    )
  );
      ...
      <main id="body-content" className="cagov-main">
        ...
        >
                        ...
                        .map((topic, index) => (
                          ...
                          >
                            <input
                              ...
                              type={"checkbox"}
                              disabled={
                                initialTopic === topic[0] ? true : false
                              }
                            />
                            <label
                              style={{
          ...
          <div className="cagov-content content-cell">
            ...
            {data.topicDisplayName && (
              <div className="topic-popular-container">
                ...
                <span
                  ...
                  style={{}}
                  dangerouslySetInnerHTML={{ __html: data.topicIcn }}
                ></span>
                ...
                <div className="topic-popular-cards-container" style={{}}>
                  {data.popularDatasets.map((dataset, index) => (
                    ...
                    >
                      ...
                      <div
                        className="topic-organization"
                        style={{ fontSize: "18px", lineHeight: "32px" }}
                      >
                        {dataset.organization}
                      </div>
                      <div className="topic-recentviews">
                        <span style={{ fontSize: "37px", lineHeight: "32px" }}>
                          {dataset.views}
                        </span>
                        <span
                          style={{
            ...
            {data.topicDisplayName ? (
              <h1 className="h3" style={{ marginTop: 0, color: "#034A6B" }}>
                Search {data.topicDisplayName} datasets
              </h1>
            ) : (
              <h1
                style={{
